...
if (!function_exists('qin_fedex_get_settings')) {
    function qin_fedex_get_settings() {
        ...
        $def = array(
            ...
            'fedex_xishu_us' => '向上取整（FEDEX_AMOUNT） * 1.04 + 重量*2',
            'fedex_xishu_other' => '向上取整（FEDEX_AMOUNT） * 1.04 + 重量*2',
...
if (!function_exists('qin_fedex_settings_page')) {
    function qin_fedex_settings_page() {
        ...
        if (!empty($_POST['qin_fedex_save'])) {
            ...
            $s['fedex_xishu_us'] = sanitize_text_field($_POST['fedex_xishu_us']);
            $s['fedex_xishu_other'] = sanitize_text_field($_POST['fedex_xishu_other']);
        ...
        <div class="wrap">
            ...
            <form method="post">
                ...
                <table class="form-table">
                    ...
                    <tr><th>样品系数（US）</th><td><input type="text" name="fedex_xishu_us" value="<?php echo esc_attr($s['fedex_xishu_us']);?>" class="regular-text"></td></tr>
                    <tr><th>样品系数（其他）</th><td><input type="text" name="fedex_xishu_other" value="<?php echo esc_attr($s['fedex_xishu_other']);?>" class="regular-text"></td></tr>
...
if (!function_exists('qin_fedex_rate_quote')) {
    function qin_fedex_rate_quote($to_country, $to_postal, $weight_kg, $args=array()) {
        ...
        if (!empty($args['city'])) {
            $recipient_addr['city'] = sanitize_text_field($args['city']); // ✅ FedEx recipient city
        }
        ...
        // 样品优先 FEDEX_PAK
        if (isset($args['is_sample']) && $args['is_sample'] == '1') {
        ...
        foreach ($details as $d) {
            ...
            if ($amount !== null) {
                ...
                if ( isset($args['is_sample']) && $args['is_sample'] == '1' ) {
                    if ( $to_country == 'US' ) {
                        $ruleStr = $s['fedex_xishu_us'];
                    } else {
                        $ruleStr = $s['fedex_xishu_other'];
                    }
                }
...
function process_shipping(WP_REST_Request $request){
    ...
    $is_sample = $request_body['is_sample'] ?? '0';
...
function qin_sample_new_checkout( WP_REST_Request $req ) {
  ...
  $is_sample    = sanitize_text_field( (string) $req->get_param('is_sample') );  
  ...
  $r->set_param('city', $city);
  $r->set_param('is_sample', $is_sample);
...
/**
 * Add this into your plugin main file (or include).
 */

add_action('rest_api_init', function () {
    register_rest_route('qinprinting/v1', '/sample_calc', [
        'methods'  => 'POST',
        'callback' => 'qinprinting_sample_calc_cb',
        'permission_callback' => '__return_true', // 前台可用；若要更严可加 nonce 校验
    ]);
});

/**
 * 可选：前端带 X-WP-Nonce，更安全一点（防 CSRF）
 */
// add_action('wp_enqueue_scripts', function () {
//     wp_localize_script('YOUR_FRONT_JS_HANDLE', 'QINPRINT', [
//         'rest'  => esc_url_raw(rest_url()),
//         'nonce' => wp_create_nonce('wp_rest'),
//     ]);
// });

function qinprinting_sample_calc_cb(WP_REST_Request $req) {
    // --- 可选 nonce 校验 ---
    ...
    // 1) 基础参数（只信任这些）
    $product_type = sanitize_key($p['product_type'] ?? 'book'); // book|card|box
    if (!in_array($product_type, ['book','card','box'], true)) $product_type = 'book';

    $unit = sanitize_key($p['unit'] ?? 'inch'); // inch|mm
    $w = floatval($p['width'] ?? 0);
    $h = floatval($p['height'] ?? 0);
    $d = floatval($p['depth'] ?? 0);
    $page_count = intval($p['page_count'] ?? 0);

    // 防止恶意超大值
    $w = qinprinting_clamp($w, 0, 2000);
    $h = qinprinting_clamp($h, 0, 2000);
    $d = qinprinting_clamp($d, 0, 2000);
    $page_count = (int)qinprinting_clamp($page_count, 0, 5000);

    // card 的 depth 规则（前端：depth = pageCount*0.4 + 3）
    ...
    // 2) 用户选择的 proof types（多选）
    // 例：["hard_proof","digital","dummy","offset"]
    $proof_types = $p['proof_types'] ?? [];
    if (!is_array($proof_types)) $proof_types = [];

    // 3) 选项（只用 code，不要用 price）
    $options = $p['options'] ?? [];
    ...
    $result = [
        ...
        'available' => true,
        'subtotal' => 0.0,
        'weight'   => 0.0, // 前端 subtotalWeightActual 起始 0.5
        'paypal_fee' => 0.0,
        'total' => 0.0,
        'breakdown' => [],
    ];

    foreach ($proof_types as $t) {
        $t = sanitize_key($t);
        switch ($t) {
            case 'hard_proof':
                $calc = qinprinting_calc_hard_proof($product_type, $w, $h, $page_count, $options['hard_proof'] ?? []);
                break;
            case 'dummy':
                $calc = qinprinting_calc_dummy($product_type, $w, $h, $page_count, $options['dummy'] ?? []);
                break;
            case 'digital':
                $calc = qinprinting_calc_digital($product_type, $w, $h, $d, $page_count, $options['digital'] ?? []);
                break;
            case 'offset':
                $calc = qinprinting_calc_offset($options['offset'] ?? []);
                break;
            default:
                $calc = ['available'=>true,'price'=>0,'weight'=>0,'note'=>'unknown type ignored'];
        }

        if (empty($calc['available'])) {
            $result['available'] = false;
            $result['breakdown'][$t] = $calc;
            break;
        }

        $price = floatval($calc['price'] ?? 0);
        $weight = floatval($calc['weight'] ?? 0);

        $result['subtotal'] += $price;
        $result['weight']   += $weight;
        $result['breakdown'][$t] = $calc;
    }
    $result['weight'] = $result['weight'] + (count($proof_types) ? 0.5 : 0);
    
    if (!$result['available']) {
        return new WP_REST_Response($result, 200);
    }

    // 5) 计费重量：向上取到 0.5kg（前端：ceil(x*2)/2）
    $chargeable_weight = qinprinting_chargeable_weight($result['weight']);
    $result['chargeable_weight'] = $chargeable_weight;

    // 6) 运费：这里建议你“复用现有运费插件”的函数/endpoint
    //    我先给一个占位：前端继续调用你现有 shippingcost 也可以；
    //    或者在这里内部调用你的运费计算函数（更安全：只让后端拿运费）
    $shipping_price = floatval($p['shipping_price'] ?? 0); // 临时兼容：建议后续去掉
    $result['shipping'] = $shipping_price;

    // 7) PayPal fee（前端：price1*0.05）
    $price1 = $result['subtotal'] + $shipping_price;
    $paypal_fee = round($price1 * 0.05, 2);
    $result['paypal_fee'] = $paypal_fee;
    $result['total'] = round($price1 + $paypal_fee, 2);
...
function qinprinting_clamp($v, $min, $max) {
    if ($v < $min) return $min;
    if ($v > $max) return $max;
    return $v;
}

function qinprinting_chargeable_weight($w) {
    if ($w <= 0) return 0;
    return ceil($w * 2) / 2;
}

/**
 * Hard Proof：按前端 hardproof 的区间计价 + weight 公式
 */
function qinprinting_calc_hard_proof($product_type, $w, $h, $page_count, $opt) {
    $w2 = min($w, $h);
    $h2 = max($w, $h);

    $count = 1;
    if ($product_type === 'book') {
        $count = intval($opt['count'] ?? $page_count);
    } elseif ($product_type === 'card') {
        $count = intval($opt['count'] ?? $page_count);
    } else {
        $count = 1;
    }
    $count = (int)qinprinting_clamp($count, 1, 5000);

    // weight：前端书/卡是面积*(+20mm)*克重*张数
    if ($product_type === 'book') {
        $weight = ((($w2 + 20) * ($h2 + 20) * $count * 200 / 1000) / 1000000);
    } elseif ($product_type === 'card') {
        $weight = ((($w2 + 20) * ($h2 + 20) * $count * 350 / 1000) / 1000000);
    } else {
        $weight = 0.5;
    }
    $weight = round($weight, 1);

    // price：前端 box 特例 10/6.6
    if ($product_type === 'box') {
        $price = 10 / 6.6;
    } else {
        if ($w2 <= 108 && $h2 <= 148) {
            $price = $count * 2.5 / 6.6 / 2;
        } elseif ($w2 <= 148.5 && $h2 <= 216) {
            $price = $count * 2.5 / 6.6;
        } elseif ($w2 <= 216 && $h2 <= 297) {
            $price = $count * 5 / 6.6;
        } else {
            $price = $count * 10 / 6.6;
        }
    }
    $price = round($price); // 前端 Math.round
...
/**
 * Dummy：前端是 “binding/base options + 多选工艺（含 places 递增）”，这里用 code 白名单
 */
function qinprinting_calc_dummy($product_type, $w, $h, $page_count, $opt) {
    // 绑定类型 code（前端是 text；后端建议用 code）
    $binding = sanitize_key($opt['binding'] ?? 'saddle');

    // 基础价（示例：按页面中 dummy binding 下拉 value：25/30 等）:contentReference[oaicite:2]{index=2}
    $binding_price_map = [
        'saddle' => 25,
        'glue_perfect' => 25,
        'sewn_perfect' => 25,
    ...
    $base = $binding_price_map[$binding] ?? 25;

    // 其他下拉（special materials / holes drilled）用 code
    $extra_map = [
        'materials_none' => 0,
        'materials_fabric' => 70,
        'materials_faux_leather' => 70,
        'materials_special_paper' => 50,
        'holes_no' => 0,
        'holes_yes' => 10,
    ];
    $extras = $opt['extras'] ?? [];
    ...
    foreach ($extras as $code) {
        ...
        $extra_fee += $extra_map[$code] ?? 0;
    }

    // 多选工艺（foil/uv/emboss…），places 采用递增规则：1个=base；>1 base + base*(n-1)/2 :contentReference[oaicite:3]{index=3}
    $surface_map = [
        'gold_silver_foil' => 60,
        'other_foil' => 110,
        'gold_silver_edge' => 160,
        'other_edge' => 190,
        'texture' => 50,
        'spot_uv' => 45,
        'debossing' => 45,
    ...
    foreach ($surface as $item) {
        ...
        $places = intval($item['places'] ?? 1);
        $places = (int)qinprinting_clamp($places, 1, 999);
        $bp = $surface_map[$code] ?? 0;
        if ($bp <= 0) continue;

        $fee = $bp;
        if ($places > 1) $fee += $bp * ($places - 1) / 2;
        $surface_fee += $fee;
    }

    $price = $base + $extra_fee + $surface_fee;

    // weight：按 bindingTypeText_dummy 算，这里做一个对应（示例，按你已有公式搬过来）
    $weight = 0;
    if (in_array($binding, ['saddle','glue_perfect','sewn_perfect'], true)) {
        $weight = ($h / 1000) * ($w / 1000) * (0.25 * 2 + 0.05 * 2 + 0.128 * $page_count / 2);
    } elseif ($binding === 'sewn_hardcover') {
        $weight = ($h / 1000) * ($w / 1000) * (0.157 * 2 + 0.05 * 2 + 0.157 * $page_count / 2 + (2/3) * 2.5 * 2.2 + 0.14 * 4);
    } elseif ($binding === 'selfcover_board') {
        $weight = ($h / 1000) * ($w / 1000) * (0.05 * 2 + (0.35 + 0.03) * $page_count);
    } elseif ($binding === 'hardcover_board') {
        $weight = ($h / 1000) * ($w / 1000) * (0.157 * 2 + 0.05 * 2 + (0.35 + 0.03) * $page_count + (2/3) * 3 * 2.2);
    }
...
/**
 * Digital：逻辑较长，这里先给“结构正确”的后端版本（核心：Z1/Z2 拼版、laborFee、extraFee、特殊最低价）
 */
function qinprinting_calc_digital($product_type, $w, $h, $d, $page_count, $opt) {
    $binding = sanitize_key($opt['binding'] ?? 'saddle');
    $bindings = [
        'glue_perfect','glue_hardcover','saddle','sewn_perfect','sewn_hardcover','selfcover_board','hardcover_board',
        // card/box 的组合可以再扩
    ];
    if (!in_array($binding, $bindings, true)) $binding = 'saddle';

    // spread 规则（胶装/胶精装=单页；其他=对裱展开）:contentReference[oaicite:4]{index=4}
    $spreadW = $w; $spreadH = $h;
    if (!in_array($binding, ['glue_perfect','glue_hardcover'], true)) {
        $spreadW = $w * 2;
    }
    $actualW = max($spreadW, $spreadH);
    $actualH = min($spreadW, $spreadH);

    $needLargeLabor = ($w > 297 || $h > 297);

    $printSheets = 0; $laborFee = 0; $extraFee = 0;

    switch ($binding) {
        case 'glue_perfect':
            $printSheets = $page_count + 4;
            $laborFee = $needLargeLabor ? 2 : 1.5;
            break;
        case 'glue_hardcover':
            $printSheets = $page_count + 8;
            $laborFee = $needLargeLabor ? 6 : 5.5;
            break;
        case 'saddle':
            $printSheets = ($page_count + 4) / 2;
            $laborFee = $needLargeLabor ? 2 : 1.5;
            break;
        case 'sewn_perfect':
            $printSheets = ($page_count + 4) / 2;
    ...
    // Z1/Z2
    $z1_a = floor(432 / $actualW) * floor(297 / $actualH);
    $z1_b = floor(297 / $actualW) * floor(432 / $actualH);
    $Z1 = max($z1_a, $z1_b);

    $z2_a = floor(730 / $actualW) * floor(510 / $actualH);
    $z2_b = floor(510 / $actualW) * floor(730 / $actualH);
    $Z2 = max($z2_a, $z2_b);

    $priceA = INF; $priceB = INF;
    if ($printSheets > 0) {
        if ($Z1 > 0) $priceA = (($printSheets * 1.85 / $Z1 / 6.6) + $laborFee) * 3 + $extraFee;
        if ($Z2 > 0) $priceB = (($printSheets * 9 / $Z2 / 6.6) + $laborFee) * 3 + $extraFee;
    }
    $minPrice = min($priceA, $priceB);

    if (!is_finite($minPrice)) {
        return ['available'=>false,'price'=>null,'weight'=>null,'note'=>'Size not available'];
    }

    // 对 book 的 board book 给固定最低价；card 用阶梯；box 自己另算
    if ($product_type === 'book') {
        if ($binding === 'selfcover_board') $minPrice = 60;
        if ($binding === 'hardcover_board') $minPrice = 70;
    } elseif ($product_type === 'card') {
        $minPrice = 30 * max(0, (int)ceil(max(0, $page_count - 100) / 100));
    }

    // extra 下拉（materials/holes/box type 等）同样用 code 白名单
    $extra_map = [
        'materials_none'=>0,'materials_fabric'=>70,'materials_faux_leather'=>70,'materials_special_paper'=>50,
        'holes_no'=>0,'holes_yes'=>10,
        'booklet_none'=>0,'booklet_perfect'=>40,'booklet_saddle'=>25,
        'card_box_none'=>40,'card_box_rigid'=>65,'card_box_tuck'=>45,
        'box_type_rigid'=>30,'box_type_tuck'=>25,
    ];
    $extras = $opt['extras'] ?? [];
    if (!is_array($extras)) $extras = [];
    $extra_fee2 = 0;
    foreach ($extras as $code) {
        $code = sanitize_key($code);
        $extra_fee2 += $extra_map[$code] ?? 0;
    }

    // surface（同 dummy）
    $surface_map = [
        'gold_silver_foil'=>60,'other_foil'=>110,'gold_silver_edge'=>160,'other_edge'=>190,
        'texture'=>50,'spot_uv'=>45,'debossing'=>45,'embossing'=>45,
    ];
    $surface_fee = 0;
    $surface = $opt['surface'] ?? [];
    if (!is_array($surface)) $surface = [];
    foreach ($surface as $item) {
        if (!is_array($item)) continue;
        $code = sanitize_key($item['code'] ?? '');
        $places = (int)qinprinting_clamp(intval($item['places'] ?? 1), 1, 999);
        $bp = $surface_map[$code] ?? 0;
        if ($bp <= 0) continue;
        $fee = $bp;
        if ($places > 1) $fee += $bp * ($places - 1) / 2;
    ...
    $price = $minPrice + $extra_fee2 + $surface_fee;
    $price = round($price, 2);

    // weight： digitalWeight 分 book/card/box 三段，这里先给 book 的那段（card/box 可继续补齐）
    $weight = 0;
    if ($product_type === 'book') {
        if (in_array($binding, ['saddle','glue_perfect','sewn_perfect'], true)) {
            $weight = ($h/1000)*($w/1000)*(0.25*2 + 0.05*2 + 0.128*$page_count/2);
        } else {
            $weight = ($h/1000)*($w/1000)*(0.157*2 + 0.05*2 + 0.157*$page_count/2 + (2/3)*3*2.2 + 0.14*4);
        }
    } elseif ($product_type === 'card') {
        $weight = 0.1; // 先占位，按公式可继续搬
    } else {
        $weight = 0.1; // 先占位
    }
    $weight = round($weight, 2);

    return ['available'=>true,'price'=>$price,'weight'=>$weight,'params'=>['binding'=>$binding]];
}

function qinprinting_calc_offset($opt) {
    //  value 直接给 100/150 等:contentReference[oaicite:5]{index=5}
    $type = sanitize_key($opt['type'] ?? 'one_sig_one_side');
    $map = [
        'one_sig_one_side' => 100,
        'one_sig_two_side' => 150,
    ];
    $price = $map[$type] ?? 100;
    return ['available'=>true,'price'=>$price,'weight'=>0.5,'params'=>['type'=>$type]];
}
// 样品数
