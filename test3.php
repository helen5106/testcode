...
function qin_sample_new_checkout( WP_REST_Request $req ) {
  ...
  // 5) Scheme B：写入 Woo session（不碰你旧的 $_SESSION['PRODUCTNOTE']）
  if ( ! WC()->session ) {
  ...
  // 6) 加入购物车：用一个固定隐藏商品（你旧逻辑用 24288，这里也可复用，不会破坏旧结构）
  $product_id = (int) apply_filters('qin_newsample_product_id', 24288);
